...
import { HomeLoaderResult } from "./home/homeLoader"

export default function HomePage() {
const { featurePacakages } = useLoaderData() as HomeLoaderResult
const rederedPackages = featurePacakages.map((p) => {
        return  <div key={p.name} className="flex flex-col justify-between gap-3 border rounded shadow p-4">
                <div className="felx flex-col gap-1 border-bottom border-gray-400">
                        <div className="font-bold text-center">{p.name}</div>
                        <div className="text-sm text-gray-500">{p.description}</div>
                        <div className="text-sm text-green-500">{p.maintainers.length}</div>
                </div>
